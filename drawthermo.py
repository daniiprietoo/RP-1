...
for i in range(n):
    for j in range(n):
        if domain[i][j] == ">" and j < n and domain[i][j + 1] == ">":
            s = "hor"
        elif domain[i][j] == "<" and j > 0 and domain[i][j - 1] == "<":
            s = "hor"
        elif domain[i][j] == "^" and i > 0 and domain[i - 1][j] == "^":
            s = "vert"
        elif domain[i][j] == "v" and i < n and domain[i + 1][j] == "v":
            s = "vert"
        else:
            s = name[domain[i][j]]
        if filled[i][j] == "x":
